...
CKEDITOR.dialog.add( 'extDialog', function( editor ) {
	...
	*/
	...
	return {
		...
		title: labels.resourcesEdit.ext.insert,
		minWidth: 400,
		minHeight: 200,
				...
				elements: [
					{
						type: 'select',
						id: 'selectLink',
						label: labels.resourcesEdit.ext.label,
						style: 'width : 100%;',
						'items': extList(),
...
/*
		...
		onOk: function() {
			...
			glossary.setAttribute("class", "external");
			
			var textOverride=dialog.getValueOf('tab-basic', 'selectLink');
            
            var list = masterStructure.resourcesManager.exts;
            
			var label = list[textOverride].description;
			
			glossary.setText(label);
            glossary.setAttribute("data-ext", textOverride);
	...
	function extList(){
        ...
        if(ids.length > 0){
            for (var i=0;i<ids.length;i++){
                //console.log(list[i]);
                returnList[returnList.length]=[objs[i].description, ids[i]];//list[i].term;
