define([
	...
	return BaseModule.extend({
		...
		initVersion: function (role) {
			var that = this;
			var allowDelete = this.setDelete(role); //for now, same rights as deleting, we can change later
			var disable = (!allowDelete) ? " disabled " : " ";
			//get course level
			this.versionLevel = this.root.version.compareVersion(this.version);
			//get class
			var classes = "course-version ico-LOM-update snap-xs";
			if (this.versionLevel >= 0) {
				classes += " LOM-versionlevel-" + this.root.version.levels[this.versionLevel];
			}
			this.$el.append("<button class='" + classes + "' " + disable + " title='Version'>X</button>");
			//lets get the version this.version





			this.$el.children(".course-version").click(function () {
				that.versionCourse();
			});
		},

		versionCourse: function () {
			if (this.versionLevel >= 0) {
				console.log(this.version);
				this.root.version.loopVersions(this.version);
			}
		},
