...
console.log('removeitem');
						...
						*/
						...
						if(targetObj.subs.length === 0){
							...
							if(targetObj.isPage){
								//Is a page → archive
								opStack.push({mPos:targetObj.sPosition,op:'archive'});
							}else{
								//Is not a page → delete
								opStack.push({mPos:targetObj.sPosition,op:'delete'});
							}
						}else{
							//Loop through subs
							for(var i in targetObj.subs){
								...
								if(targetObj.subs[i].subs.length === 0){
									...
									if(targetObj.subs[i].isPage){
										opStack.push({mPos:targetObj.subs[i].sPosition,op:'archive'});
									}else{
										opStack.push({mPos:targetObj.subs[i].sPosition,op:'delete'});
									}
								}else{
									...
									for(var ii in targetObj.subs[i].subs){
										...
										if(targetObj.subs[i].subs[ii].isPage){
											opStack.push({mPos:targetObj.subs[i].subs[ii].sPosition,op:'archive'});
										}else{
											opStack.push({mPos:targetObj.subs[i].subs[ii].sPosition,op:'delete'});
										}
									}
